<template>
  <div class="tw-h-full">
    ...
    >
        <div v-if="visible" v-click-outside="() => visible = false" class="tw-absolute tw-right-0 tw-z-10">
            <div class="tw-absolute tw-top-1/2 -tw-translate-y-1/2 tw-right-10 tw-bg-white tw-border tw-border-solid  tw-p-2 tw-rounded tw-flex tw-items-center tw-gap-2">
                ...
                <button title="Show history" v-if="$user.role == 'admin'" @click="HistoryPopup = true" class="tw-aspect-square tw-p-0.5 tw-w-[32px] tw-flex tw-items-center tw-justify-center tw-rounded tw-border tw-border-solid tw-border-teal-100 hover:tw-border-teal-400 tw-bg-teal-50/10 hover:tw-bg-teal-100 tw-duration-200">
                    <icon icon="uim:history" class="tw-text-xl tw-text-teal-500" />
                </button>
                
                <a title="Download Excel File" v-if="$user.role == 'admin'" target="_blank" :href="serverUrl + 'api/export/factorisations/' + item.factorisation_id" class="tw-aspect-square tw-p-0.5 tw-w-[32px] tw-flex tw-items-center tw-justify-center tw-rounded tw-border tw-border-solid tw-border-green-100 hover:tw-border-green-400 tw-bg-green-50/10 hover:tw-bg-green-200 tw-duration-200">
                    <icon icon="vscode-icons:file-type-excel" class="tw-text-xl" />
                </a>

                <button title="Add comment" v-if="$user.role == 'admin' && false" @click="CommentPopup = true" class="tw-aspect-square tw-p-0.5 tw-w-[32px] tw-flex tw-items-center tw-justify-center tw-rounded tw-border tw-border-solid tw-border-blue-100 hover:tw-border-blue-400 tw-bg-blue-50/10 hover:tw-bg-blue-100 tw-duration-200">
                    <icon icon="solar:notes-bold-duotone" class="tw-text-xl tw-text-blue-500" />
                </button>

                <button title="Edit" v-if="$user.role == 'admin' && false" @click="popup = true" class="tw-aspect-square tw-p-0.5 tw-w-[32px] tw-flex tw-items-center tw-justify-center tw-rounded tw-border tw-border-solid tw-border-amber-100 hover:tw-border-amber-400 tw-bg-amber-50/10 hover:tw-bg-amber-100 tw-duration-200">
                    <icon icon="lets-icons:edit-duotone" class="tw-text-xl tw-text-amber-500" />
                </button>
                <button title="Delete" v-if="$user.role == 'admin'" @click="DeletePopup = true" class="tw-aspect-square tw-p-0.5 tw-w-[32px] tw-flex tw-items-center tw-justify-center tw-rounded tw-border tw-border-solid tw-border-rose-100 hover:tw-border-rose-400 tw-bg-rose-50/10 hover:tw-bg-rose-100 tw-duration-200">
    ...
    <button title="Add fees" v-if="$user.role == 'admin'" @click="FeesPopup = true" class="tw-aspect-square tw-p-0.5 tw-w-[32px] tw-flex tw-items-center tw-justify-center tw-rounded tw-border tw-border-solid tw-border-emerald-100 hover:tw-border-emerald-400 tw-bg-emerald-50/10 hover:tw-bg-emerald-100 tw-duration-200">
        <icon icon="solar:bill-list-bold-duotone" class="tw-text-xl tw-text-emerald-500" />
    </button>

    <TableActionsUploadImage title="Upload image" v-if="$user.role == 'admin'" @update="newItem => $emit('update', newItem)" :key="item.id" :item="item" />
