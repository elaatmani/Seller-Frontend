<template>
  <div class="tw-border tw-bg-white tw-w-full tw-rounded-md tw-mb-3">
    <div
      class="tw-p-1 tw-bg-gray-100 tw-rounded tw-w-full md:tw-flex tw-flex-wrap tw-justify-center md:tw-justify-between">
      <div class=" tw-w-full md:tw-w-fit tw-mb-2 md:tw-mb-0">
        <button class="tw-bg-white tw-border tw-border-gray-300 tw-rounded tw-w-full tw-px-2 tw-py-2 tw-shadow-sm"
      ...
      <div v-if="showDatePicker"
        class="tw-z-50 tw-bg-white tw-rounded tw-shadow-lg tw-p-4 md:tw-mt-12 tw-mt-0 tw-w-3/4 md:tw-w-1/4 tw-absolute">
        <div class="tw-w-fit tw-flex tw-justify-center md:tw-flex-wrap" v-show="!isMobile">
          <button v-for="option in dateOptions" :key="option.value"
        ...
        <select
          ...
          v-show="isMobile" v-model="selectedRange" @change="selectDateRange(selectedRange)">
        ...
        <div class="tw-flex-1">
          ...
          <div class="tw-flex md:tw-justify-end">
            <button class="tw-bg-gray-200 tw-border tw-border-gray-300 tw-rounded tw-px-4 tw-py-2 tw-mr-2"
      ...
      <div class="tw-flex tw-flex-wrap">
        <div class="tw-w-full md:tw-w-[200px] tw-mb-2 md:tw-mb-0 md:tw-mr-2 tw-mr-0 ">
          <vue-select :multiple="true" :reduce="product => product.id" :clearable="false"
            ...
            v-model="selectedProductId" :options="productOptions" label="name" placeholder="Select a product">
          </vue-select>
      ...
      <div class="tw-flex tw-flex-wrap">
        <div class="tw-w-full md:tw-w-[200px] tw-mb-2 md:tw-mb-0 md:tw-mr-2 tw-mr-0 ">
          <vue-select :multiple="true" :reduce="agente => agente.agente_id" :clearable="false"
            class="tw-bg-white tw-items-center tw-border-solid tw-outline-none tw-text-gray-900 tw-text-sm tw-rounded-lg focus:tw-ring-orange-500 focus:tw-border-orange-500 tw-block md:tw-w-[200px] tw-w-full"
            v-model="selectedAgenteId" :options="[...agenteOptions]" label="name" placeholder="Select a agente">
          </vue-select>
        </div>

        <div class="tw-w-full md:tw-w-fit tw-mb-2 md:tw-mb-0 md:tw-mr-2 tw-mr-0">
          <button class="tw-bg-orange-500 tw-text-white tw-rounded tw-px-4 tw-h-[33px] md:tw-ml-2 tw-ml-0 tw-w-full"
            @click="send">Filter</button>
        </div>
      </div>
    </div>
  </div>
...
const products = computed(() => store.getters['product/products']); // Assuming products are stored in Vuex
const productOptions = computed(() => products.value.map(({ id, name }) => ({ id, name })));
...
const send = () => {
  emit('filter-changed', {
    ...
    filter: {
      selectedAgenteId: selectedAgenteId.value,
      selectedProductId: selectedProductId.value // Add product filter
    }
  });
  //  compareFbTiktok();
};
