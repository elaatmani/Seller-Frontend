...
const getData = async (per_page = 6, page = 1) => {
    ...
    await Analytics.getOrdersBySellers({ per_page, page, order_by: order_by.value, from: filters.value.date.start, to: filters.value.date.end, sellers: filters.value.sellers })
        .then(
            res => {
