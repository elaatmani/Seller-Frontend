                ...
                <div class="tw-w-full tw-h-fit tw-bg-white tw-rounded-md tw-border tw-border-solid tw-shadow-md tw-overflow-hidden">
                    ...
                    <div class="tw-p-5 tw-space-y-4">
                        ...
                        <div v-if="product_variations.length > 1">
                            ...
                            <div class="tw-relative">
                                <select 
                                :disabled="!productsFetched || form.request.product_id == 'no-value-selected'"
                        ...
                        <div>
                            ...
                            <input v-model="form.request.quantity" @input="form.errors.quantity = null" type="number" min="0" max="10000"
                        ...
                        <div>
                            <label
                                class="tw-block tw-mb-2 tw-text-sm tw-font-medium tw-text-gray-900 dark:tw-text-white">Message</label>
                            <textarea v-model="form.request.note" @input="form.errors.note = null" type="number" min="0" max="10000"
                                :class="[form.errors.note && '!tw-border-red-400']"
                                class="tw-bg-gray-50 tw-border tw-border-solid focus:tw-outline-none tw-border-gray-300 tw-text-gray-900 tw-text-sm tw-rounded-lg focus:tw-ring-orange-500 focus:tw-border-orange-500 tw-block tw-w-full tw-p-2.5 dark:tw-bg-gray-700 dark:tw-border-gray-600 dark:tw-placeholder-gray-400 dark:tw-text-white dark:focus:tw-ring-orange-500 dark:focus:tw-border-orange-500"
                                placeholder="Enter a note for your request" rows="2" required ></textarea>
...
const form = reactive({
    request: {
        product_id: 'no-value-selected',
        product_variation_id: 'no-value-selected',
        quantity: 0,
        note: ''
    },
    errors: {}
...
const create = async () => {
    loading.value = true;
    await SupplyRequest.create(form.request)
    .then(
        res => {
            if(res.data.code == 'SUCCESS') {
                useAlert('Request has been added');
                store.dispatch('supply-request/addSupplyRequest', res.data.supply_request);
                hide();
            }
        },
        err => {
