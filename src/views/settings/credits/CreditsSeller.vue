<template>
    <div class="py-5 px-2 tw-border tw-bg-white tw-w-full tw-rounded-md tw-mb-5 ">
      ...
      <div class="tw-flex tw-items-center tw-justify-between tw-flex-wrap">
          <div>
              <div class="tw-flex tw-items-center tw-gap-x-3">
                  <h2 class="tw-text-lg tw-font-medium tw-text-gray-800 darkx:tw-text-white">Credits</h2>
  
                  <span class="tw-px-3 tw-py-1 tw-text-xs tw-text-emerald-600 tw-bg-emerald-100 tw-rounded-full darkx:tw-bg-gray-800 darkx:tw-text-orange-400">{{ totalOrders }} credits</span>
              </div>
          </div>
      ...
      <div>
        ...
        <div v-if="create_popup">
          <CreatePopup  :filters="filters" v-model:visible="create_popup" />
        </div>
  ...
  import IndexTable from '@/views/settings/credits/partials/IndexTable'
  import IndexFilters from '@/views/settings/credits/partials/filters/IndexFilters';
  import CreatePopup from '@/views/settings/credits/partials/components/CreatePopup'
  ...
  export default {
    ...
    methods: {
      ...
      paginateOrders() {
        ...
        return Credit.paginate(url, options)
        .then(({data}) => {
          const options = data.data.Credit;
