    ...
    <div v-else class="data-visualization">
      ...
      <template v-else>
        ...
        <div class="capsule-list">
          <div
            v-for="(reason, index) in paginatedReasons"
            :key="index"
            class="data-capsule group"
            :style="{
              '--progress': `${(reason.orders / max) * 100}%`,
              '--color-rgb': colors[originalIndex(index)],
            }"
          >
...
const paginatedReasons = computed(() => {
  ...
  return reasons.value.slice(start, end);
});

const totalPages = computed(() => Math.ceil(reasons.value.length / itemsPerPage));
const showPagination = computed(() => reasons.value.length > itemsPerPage);
const originalIndex = (pageIndex) => (currentPage.value - 1) * itemsPerPage + pageIndex;
...
const getData = async (productId = null) => {
  ...
  try {
    ...
    if (res.data.code === "SUCCESS") {
      ...
      currentPage.value = 1;  // Reset to first page on data change
...
.pagination-controls {
  @apply tw-flex tw-justify-center tw-items-center tw-gap-4 tw-mt-4;
}

.prev-button, .next-button {
  @apply tw-px-4 tw-py-2 tw-bg-gray-100 tw-rounded-lg hover:tw-bg-gray-200 
         tw-transition-colors tw-text-sm tw-font-medium tw-text-gray-700;
}

button:disabled {
  @apply tw-opacity-50 tw-cursor-not-allowed hover:tw-bg-gray-100;
}

.page-info {
  @apply tw-text-sm tw-text-gray-600 tw-mx-2;
}
